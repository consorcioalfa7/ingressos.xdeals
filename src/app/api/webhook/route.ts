...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (apiKey !== process.env.NEXUS_API_KEY) {
      console.error('[Security] Tentativa de webhook não autorizada.');
    ...
    switch (body.status) {
      ...
      default:
        console.log(`[Webhook] Unknown status: ${body.status}`);
    ...
    // ==========================================
    // 7. GENERATE TICKETS (IF PAID)
    // ==========================================

    if (shouldGenerateTickets) {
      console.log(`[Webhook] Generating tickets for order ${order.orderId}`);
      ...
      for (const item of order.items) {
        ...
        // Update sold count and release reservation
      ...
      console.log(`[Webhook] Generated ${tickets.length} tickets for order ${order.orderId}`);

      // Send confirmation with tickets
      await sendPaymentConfirmationNotification(order, body.amount_paid || order.total, tickets);
    } else {
...
async function sendPaymentConfirmationNotification(
  ...
  try {
    // Send purchase confirmation using notification service
    ...
    // Also log ticket codes for immediate reference
...
`.trim();
    ...
    console.log('[Notification] Payment confirmed:', order.orderId);
    console.log(message);
...
`.trim();
    ...
    console.log('[Notification] Payment expired:', order.orderId);
    console.log(message);
  } catch (error) {
    console.error('[Notification] Failed to send expiration notice:', error);
  }
}
